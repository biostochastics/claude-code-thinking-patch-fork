...
// ============================================================
// Claude Code Thinking Custom Peach Style Patcher v2.1.74
...
// Gate function: TGY (gates whether thinking blocks render at all)
...
// TGY function patterns - this function gates whether thinking blocks are rendered at all
...
const TGYSearchPattern = 'case"redacted_thinking":{if(!D&&!w)return null;';
const TGYReplacement = 'case"redacted_thinking":{if(!1)return null;';
const TGYThinkingSearchPattern = 'case"thinking":{if(!D&&!w)return null;';
const TGYThinkingReplacement = 'case"thinking":{if(!1)return null;';
...
const kv1CustomReplacement = 'function kv1(A){let q=A6(11),{param:K,addMargin:Y,isTranscriptMode:z,verbose:_,hideInTranscript:w}=A,{thinking:O}=K,$=Y===void 0?!1:Y,H=w===void 0?!1:w,j=yq("app:toggleTranscript","Global","ctrl+o");if(!O)return null;if(!1)return null;if(!1){let W=$?1:0,f=`${"\u2234 Thinking"} (${j} to expand)`,Z;if(q[0]!==f)Z=NY6.default.createElement(T,{dimColor:!0,italic:!0},f),q[0]=f,q[1]=Z;else Z=q[1];let G;if(q[2]!==W||q[3]!==Z)G=NY6.default.createElement(m,{marginTop:W},Z),q[2]=W,q[3]=Z,q[4]=G;else G=q[4];return G}let M=$?1:0,D;if(q[5]===Symbol.for("react.memo_cache_sentinel"))D=NY6.default.createElement(T,{color:"warning",bold:!0},"\ud83c\udf51 Thinking Process"),q[5]=D;else D=q[5];let X;if(q[6]!==O)X=NY6.default.createElement(m,{paddingLeft:1,marginTop:1},NY6.default.createElement(d_,null,O)),q[6]=O,q[7]=X;else X=q[7];let P;if(q[8]!==M||q[9]!==X)P=NY6.default.createElement(m,{flexDirection:"column",borderStyle:"single",borderColor:"warning",paddingX:1,marginTop:M,overflow:"hidden"},D,X),q[8]=M,q[9]=X,q[10]=P;else P=q[10];return P}';
...
const kv1OldCustomReplacement = 'borderColor:"#FFA500",paddingX:1,marginTop:M,width:"100%"';

function checkPatchStatus(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');

    // Check TGY patch status (the outer gate that controls if thinking blocks render at all)
    const tgyPatched = content.includes(TGYReplacement) && content.includes(TGYThinkingReplacement);
    const tgyNeedsPatching = content.includes(TGYSearchPattern) || content.includes(TGYThinkingSearchPattern);

    // Check for new custom peach patched version (this script's target with theme colors)
    if (content.includes(kv1CustomReplacement)) {
      if (tgyPatched) {
        return { status: 'patched', canPatch: false, variant: 'custom-peach-v3' };
      } else if (tgyNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'custom-peach-v2-no-tgy', note: 'Has kv1 patch but missing TGY gate patch (thinking hidden by default)' };
      }
    ...
    if (content.includes(kv1OldCustomReplacement) && content.includes('\ud83c\udf51 Thinking Process')) {
    ...
    // Check for any other patch variant (has the if(!1) pattern in kv1 but different styling)
    if (content.includes('function kv1(A)') && content.match(/function kv1\(A\)[^}]*if\(!1\)return null/)) {
      if (tgyNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'partial-no-tgy', note: 'kv1 patched but TGY gate not patched (thinking hidden by default)' };
      }
...
// Old kv1 custom replacement (with hex colors - for upgrading)
const kv1OldCustomReplacementFull = 'function kv1(A){let q=A6(11),{param:K,addMargin:Y,isTranscriptMode:z,verbose:_,hideInTranscript:w}=A,{thinking:O}=K,$=Y===void 0?!1:Y,H=w===void 0?!1:w,j=yq("app:toggleTranscript","Global","ctrl+o");if(!O)return null;if(!1)return null;if(!1){let W=$?1:0,f=`${"\u2234 Thinking"} (${j} to expand)`,Z;if(q[0]!==f)Z=NY6.default.createElement(T,{dimColor:!0,italic:!0},f),q[0]=f,q[1]=Z;else Z=q[1];let G;if(q[2]!==W||q[3]!==Z)G=NY6.default.createElement(m,{marginTop:W},Z),q[2]=W,q[3]=Z,q[4]=G;else G=q[4];return G}let M=$?1:0,D;if(q[5]===Symbol.for("react.memo_cache_sentinel"))D=NY6.default.createElement(T,{color:"#FFA500",bold:!0},"\ud83c\udf51 Thinking Process"),q[5]=D;else D=q[5];let X;if(q[6]!==O)X=NY6.default.createElement(m,{paddingLeft:1,marginTop:1},NY6.default.createElement(d_,null,O)),q[6]=O,q[7]=X;else X=q[7];let P;if(q[8]!==M||q[9]!==X)P=NY6.default.createElement(m,{flexDirection:"column",borderStyle:"single",borderColor:"#FFA500",paddingX:1,marginTop:M,width:"100%"},D,X),q[8]=M,q[9]=X,q[10]=P;else P=q[10];return P}';
...
function applyPatch(installation) {
  ...
  let modified = false;
  let tgyPatched = false;

  // First, always check and apply TGY patches (the outer gate)
  if (content.includes(TGYSearchPattern)) {
    content = content.replace(TGYSearchPattern, TGYReplacement);
    tgyPatched = true;
    console.log('   \u2705 TGY redacted_thinking gate patched: if(!D&&!w) -> if(!1)');
  }
  if (content.includes(TGYThinkingSearchPattern)) {
    content = content.replace(TGYThinkingSearchPattern, TGYThinkingReplacement);
    tgyPatched = true;
    console.log('   \u2705 TGY thinking gate patched: if(!D&&!w) -> if(!1)');
  }

  // Check if kv1 is already fully patched with custom peach style
  if (content.includes(kv1CustomReplacement)) {
    if (tgyPatched) {
      fs.writeFileSync(installation.path, content, 'utf8');
      console.log('   \u2705 TGY gate patches applied (kv1 was already patched)');
      return true;
    }
  ...
  if (content.includes(kv1OldCustomReplacementFull)) {
    content = content.replace(kv1OldCustomReplacementFull, kv1CustomReplacement);
    modified = true;
    console.log('   \u2705 kv1 upgraded from hex colors to theme colors!');
  }
  // Can patch from standard version too
  else if (content.includes(kv1StandardReplacement)) {
    content = content.replace(kv1StandardReplacement, kv1CustomReplacement);
    modified = true;
    console.log('   \u2705 kv1 upgraded from standard to custom peach style!');
  }
  // Patch fresh from original
  else if (content.includes(kv1SearchPattern)) {
    content = content.replace(kv1SearchPattern, kv1CustomReplacement);
    modified = true;
    console.log('   \u2705 kv1 patch applied!');
  ...
  if (modified || tgyPatched) {
    fs.writeFileSync(installation.path, content, 'utf8');
    if (!modified && tgyPatched) {
      console.log('   \u2705 TGY gate patches applied successfully!');
    }
    return true;
  } else {
...
async function main() {
  console.log('Claude Code Thinking Custom Peach Style Patcher v2.1.74');
  console.log('========================================================');
