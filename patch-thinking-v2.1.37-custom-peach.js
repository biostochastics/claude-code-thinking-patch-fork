...
// ============================================================
// Claude Code Thinking Custom Peach Style Patcher v2.1.37
...
// Gate function: iGY (gates whether thinking blocks render at all)
...
// iGY function patterns - this function gates whether thinking blocks are rendered at all
...
const iGYSearchPattern = 'case"redacted_thinking":{if(!j&&!Z)return null;';
const iGYReplacement = 'case"redacted_thinking":{if(!1)return null;';
const iGYThinkingSearchPattern = 'case"thinking":{if(!j&&!Z)return null;';
const iGYThinkingReplacement = 'case"thinking":{if(!1)return null;';
...
const Mj6CustomReplacement = 'function Mj6(A){let q=A1(17),{param:K,addMargin:Y,isTranscriptMode:z,hideInTranscript:w}=A,{thinking:H}=K,$=Y===void 0?!1:Y,O=w===void 0?!1:w,_=LK("app:toggleTranscript","Global","ctrl+o"),J;if(q[0]===Symbol.for("react.memo_cache_sentinel"))J=!1,q[0]=J;else J=q[0];if(!H&&!J)return null;if(!1)return null;let D=z,j;if(q[1]!==H)j="\u2234 Thinking",q[1]=H,q[2]=j;else j=q[2];let M=j;if(!1){let Z=$?1:0,N;if(q[3]!==_||q[4]!==M)N=y31.default.createElement(f,{dimColor:!0,italic:!0},M," (",_," to expand)"),q[3]=_,q[4]=M,q[5]=N;else N=q[5];let T;if(q[6]!==Z||q[7]!==N)T=y31.default.createElement(I,{marginTop:Z},N),q[6]=Z,q[7]=N,q[8]=T;else T=q[8];return T}let W=$?1:0,G;if(q[9]!==M)G=y31.default.createElement(f,{color:"warning",bold:!0},"\u{1F351} Thinking Process"),q[9]=M,q[10]=G;else G=q[10];let P;if(q[11]!==H)P=y31.default.createElement(I,{paddingLeft:1,marginTop:1},y31.default.createElement(MJ,null,H)),q[11]=H,q[12]=P;else P=q[12];let V;if(q[13]!==W||q[14]!==G||q[15]!==P)V=y31.default.createElement(I,{flexDirection:"column",borderStyle:"single",borderColor:"warning",paddingX:1,marginTop:W,overflow:"hidden"},G,P),q[13]=W,q[14]=G,q[15]=P,q[16]=V;else V=q[16];return V}';
...
const Mj6OldCustomReplacement = 'borderColor:"#FFA500",paddingX:1,marginTop:W,width:"100%"';

function checkPatchStatus(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');

    // Check iGY patch status (the outer gate that controls if thinking blocks render at all)
    const igyPatched = content.includes(iGYReplacement) && content.includes(iGYThinkingReplacement);
    const igyNeedsPatching = content.includes(iGYSearchPattern) || content.includes(iGYThinkingSearchPattern);
    ...
    if (content.includes(Mj6CustomReplacement)) {
      ...
      } else if (igyNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'custom-peach-v2-no-igy', note: 'Has Mj6 patch but missing iGY gate patch (thinking hidden by default)' };
      }
    ...
    if (content.includes(Mj6OldCustomReplacement) && content.includes('\u{1F351} Thinking Process')) {
    ...
    // Check for any other patch variant (has the if(!1) pattern in Mj6 but different styling)
    if (content.includes('function Mj6(A)') && content.match(/function Mj6\(A\)[^}]*if\(!1\)return null/)) {
      if (igyNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'partial-no-igy', note: 'Mj6 patched but iGY gate not patched (thinking hidden by default)' };
      }
...
// Old Mj6 custom replacement (with hex colors - for upgrading)
const Mj6OldCustomReplacementFull = 'function Mj6(A){let q=A1(17),{param:K,addMargin:Y,isTranscriptMode:z,hideInTranscript:w}=A,{thinking:H}=K,$=Y===void 0?!1:Y,O=w===void 0?!1:w,_=LK("app:toggleTranscript","Global","ctrl+o"),J;if(q[0]===Symbol.for("react.memo_cache_sentinel"))J=!1,q[0]=J;else J=q[0];if(!H&&!J)return null;if(!1)return null;let D=z,j;if(q[1]!==H)j="\u2234 Thinking",q[1]=H,q[2]=j;else j=q[2];let M=j;if(!1){let Z=$?1:0,N;if(q[3]!==_||q[4]!==M)N=y31.default.createElement(f,{dimColor:!0,italic:!0},M," (",_," to expand)"),q[3]=_,q[4]=M,q[5]=N;else N=q[5];let T;if(q[6]!==Z||q[7]!==N)T=y31.default.createElement(I,{marginTop:Z},N),q[6]=Z,q[7]=N,q[8]=T;else T=q[8];return T}let W=$?1:0,G;if(q[9]!==M)G=y31.default.createElement(f,{color:"#FFA500",bold:!0},"\u{1F351} Thinking Process"),q[9]=M,q[10]=G;else G=q[10];let P;if(q[11]!==H)P=y31.default.createElement(I,{paddingLeft:1,marginTop:1},y31.default.createElement(MJ,null,H)),q[11]=H,q[12]=P;else P=q[12];let V;if(q[13]!==W||q[14]!==G||q[15]!==P)V=y31.default.createElement(I,{flexDirection:"column",borderStyle:"single",borderColor:"#FFA500",paddingX:1,marginTop:W,width:"100%"},G,P),q[13]=W,q[14]=G,q[15]=P,q[16]=V;else V=q[16];return V}';
...
function applyPatch(installation) {
  ...
  let modified = false;
  let igyPatched = false;

  // First, always check and apply iGY patches (the outer gate)
  if (content.includes(iGYSearchPattern)) {
    content = content.replace(iGYSearchPattern, iGYReplacement);
    igyPatched = true;
    console.log('   \u2705 iGY redacted_thinking gate patched: if(!j&&!Z) -> if(!1)');
  }
  if (content.includes(iGYThinkingSearchPattern)) {
    content = content.replace(iGYThinkingSearchPattern, iGYThinkingReplacement);
    igyPatched = true;
    console.log('   \u2705 iGY thinking gate patched: if(!j&&!Z) -> if(!1)');
  }

  // Check if Mj6 is already fully patched with custom peach style
  if (content.includes(Mj6CustomReplacement)) {
    if (igyPatched) {
      fs.writeFileSync(installation.path, content, 'utf8');
      console.log('   \u2705 iGY gate patches applied (Mj6 was already patched)');
      return true;
    }
  ...
  if (content.includes(Mj6OldCustomReplacementFull)) {
    content = content.replace(Mj6OldCustomReplacementFull, Mj6CustomReplacement);
    modified = true;
    console.log('   \u2705 Mj6 upgraded from hex colors to theme colors!');
  }
  // Can patch from standard version too
  else if (content.includes(Mj6StandardReplacement)) {
    content = content.replace(Mj6StandardReplacement, Mj6CustomReplacement);
    modified = true;
    console.log('   \u2705 Mj6 upgraded from standard to custom peach style!');
  }
  // Patch fresh from original
  else if (content.includes(Mj6SearchPattern)) {
    content = content.replace(Mj6SearchPattern, Mj6CustomReplacement);
    modified = true;
    console.log('   \u2705 Mj6 patch applied!');
  ...
  if (modified || igyPatched) {
    fs.writeFileSync(installation.path, content, 'utf8');
    if (!modified && igyPatched) {
      console.log('   \u2705 iGY gate patches applied successfully!');
    }
    return true;
  } else {
...
async function main() {
  console.log('Claude Code Thinking Custom Peach Style Patcher v2.1.37');
  console.log('========================================================');
