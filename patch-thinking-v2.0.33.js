...
/**
 * Thinking Visibility Patch for Claude Code v2.0.33
 *
 * This script patches the Claude Code CLI to restore visibility of <thinking> blocks.
 *
 * In v2.0.33, thinking content is hidden when not in transcript mode (!V) and not verbose (!I).
 ...
 *
 * Component identifier: sh6 (the thinking display component)
 */
...
console.log('Claude Code v2.0.33 Thinking Visibility Patch');
...
if (!content.includes(searchPattern)) {
  if (content.includes(replacePattern) &&
      content.includes('function sh6({param:')) {
    ...
    console.error('');
    console.error('This patch is designed for Claude Code v2.0.33.');
...
console.log('');
console.log('The sh6 component (thinking display) will now render thinking content.');
console.log('');
