...
|---------|-----------|-------|-----|------|-----------------|------|---------|
| **v2.1.121** | `K96` | `c$H` | `p` | `v` | `fY` | `dW5` | npm (binary) |
...
v2.1.30+ has a gate function (IMY/EPY/iGY/fyY/jiY/ZgY/TcY/TGY/oTY/ty_/Tpz/EQz/Udz/eOY/d9Y/m7Y/wKY/ni1/RH5/ZJ5/dW5) that controls whether thinking blocks render at all.
...
**Standard identifier rotation — same Bun SEA architecture as v2.1.113/v2.1.116/v2.1.119.**

v2.1.121 keeps the binary format; the JS bundle still appears as two byte-identical copies inside the `__BUN` segment (offset 72,155,136 / +123,294,720 on the macOS arm64 build — slightly shifted from v2.1.119). All six patches from v2.1.119's custom-peach variant port over one-to-one after identifier rotation — every search/replace pair retained the same byte length, so the same-length byte-patch + ad-hoc `codesign` workflow continues to work.

**Redact-thinking beta header:**
```javascript
// In beta header builder function:
if(O&&e89(H)&&!S8()&&I8().showThinkingSummaries!==!0)_.push(CI_);
// CI_ = "redact-thinking-2026-02-12"
// ← Patch changes O to 0 (same length) so condition is always false
```

**Function signature:**
```javascript
function K96(H){let _=VP7.c(9),{param:q,addMargin:K,isTranscriptMode:O,verbose:T,hideInTranscript:A}=H,{thinking:$}=q,z=K===void 0?!1:K,Y=A===void 0?!1:A;
  if(!$)return null;
...
**Gate function (dW5):**
```javascript
case"redacted_thinking":{if(!J&&!A)return null;  // ← Custom-peach patch: !J&&!A -> !1&&!1
case"thinking":{if(!J&&!A)return null;           // ← Custom-peach patch: same
```

**Key identifiers:**
- Component: `K96` | React: `c$H` | Box: `p` | Text: `v` | ThinkingContent: `fY`
- Gate: `dW5` | Spinner sub-component: `Uf` | Cache hook: `VP7.c(9)` (9 slots)
- Redact header var: `CI_` = `"redact-thinking-2026-02-12"`

**Notable changes from v2.1.119:**
- Component renamed: `_86` → `K96`
- React import: `j$H` → `c$H`
- Text: `k` → `v`
- ThinkingContent: `rz` → `fY`
- Spinner: `qj` → `Uf`
- Cache: `Ij7.c(9)` → `VP7.c(9)` (still 9 slots)
- Gate: `ZJ5` → `dW5`, conditional variables renamed (`!f&&!$` → `!J&&!A`)
- Redact header variable: `EE_` → `CI_`
- Redact conditional helpers: `K_9` → `e89`, `h8` → `S8`, `x8` → `I8`
- Destructuring swap inside K96: `hideInTranscript:$,thinking:A` → `hideInTranscript:A,thinking:$`
- Live-branch local vars: `M,D,j,J` → `M,D,j,J` for first-branch and dead-code, but in the live render path the thinking element variable is `J` (was `f`) and the final returned element is `f` (was `J`)
- Box (`p`) unchanged — first time Box has held its name for two consecutive releases since v2.1.116→v2.1.119

**6th patch (custom-peach only) — force `thinking.display="summarized"`:**

Same mechanism as v2.1.119 — the server still defaults thinking display to `"omitted"` (empty thinking text) unless the client explicitly sets `display:"summarized"`. v2.1.121 renamed the flow variable controlling thinking-config:

```javascript
// Original (22 bytes):
$_=U_?q.display:void 0
// Patched (22 bytes, 7 trailing spaces for length parity):
$_="summarized"
```

`$_` feeds into both `J_={type:"adaptive",display:$_}` and `J_={type:"enabled",...,display:$_}`, so both thinking-config paths now request summarized output. Variable rename from v2.1.119: `eH` → `U_`.
...
| Date | Version | Notes |
|------|---------|-------|
| 2026-04-28 | **v2.1.121** | New identifiers (K96/c$H/dW5), pure rotation, gate fix, peach styling, force `thinking.display="summarized"` |
| 2026-04-24 | v2.1.119 | New identifiers (_86/j$H/ZJ5), pure rotation, gate fix, peach styling, force `thinking.display="summarized"` |
