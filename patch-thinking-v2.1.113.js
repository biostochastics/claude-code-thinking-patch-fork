...
// ============================================================
// Claude Code Thinking Display Patcher v2.1.113 (Standard)
//
// FORMAT CHANGE: v2.1.113 ships a native Bun SEA binary
// (bin/claude.exe) instead of a cli.js. This patcher applies
// SAME-LENGTH byte replacements inside the binary and re-signs
// it on macOS (ad-hoc signature).
//
// Identifiers: Yl_ (component), g1H (React), m (Box), L (Text),
//   SA (ThinkingContent), R_7 (cache, 9 slots), iz (Spinner)
// Gate function: ni1 (not patched in standard — see -custom-peach)
...
function getInstallationPaths() {
  ...
  // 1. ~/.claude/local/ — legacy cli.js installs from older `claude update`
  ...
  // 2. nvm-managed global binary (most common install path for v2.1.113)
  ...
  // 3. npm root -g/@anthropic-ai/claude-code/bin/claude.exe
  ...
  // 4. Resolve whichever `claude` is first on PATH
...
// Same-length byte patches for v2.1.113 binary.
...
const PATCHES = [
  ...
  },
  {
    ...
    desc: 'if(w)return null -> if(0) ; if(!(O||T)) -> if(!(1||1))',
...
function checkPatchStatus(inst) {
  try {
    ...
    // Sanity: must contain the v2.1.113 Yl_ signature
    const v2113Marker = Buffer.from('function Yl_(H){let _=R_7.c(9)');
    if (data.indexOf(v2113Marker) === -1) {
...
async function main() {
  console.log('Claude Code Thinking Display Patcher v2.1.113 (Standard)');
  console.log('========================================================');
  ...
  console.log('   - Yl_ component always renders expanded thinking (no compact spinner view)');
  console.log('\nNote: The outer ni1 gate still returns null in default mode — thinking');
  console.log('      blocks appear only in transcript mode (ctrl+o) or with --verbose.');
  console.log('      For default-mode visibility use patch-thinking-v2.1.113-custom-peach.js.');
}
