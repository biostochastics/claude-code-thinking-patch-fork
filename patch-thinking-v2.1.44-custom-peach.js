...
// ============================================================
// Claude Code Thinking Custom Peach Style Patcher v2.1.44
...
// Gate function: fyY (gates whether thinking blocks render at all)
...
// fyY function patterns - this function gates whether thinking blocks are rendered at all
// The check `if(!D&&!Z)return null` blocks rendering when not in transcript mode
// We need to change both occurrences (for redacted_thinking and thinking cases) to if(!1)
const fyYSearchPattern = 'case"redacted_thinking":{if(!D&&!Z)return null;';
const fyYReplacement = 'case"redacted_thinking":{if(!1)return null;';
const fyYThinkingSearchPattern = 'case"thinking":{if(!D&&!Z)return null;';
const fyYThinkingReplacement = 'case"thinking":{if(!1)return null;';
...
const dW6CustomReplacement = 'function dW6(A){let q=e(17),{param:K,addMargin:Y,isTranscriptMode:z,hideInTranscript:w}=A,{thinking:H}=K,$=Y===void 0?!1:Y,O=w===void 0?!1:w,_=fK("app:toggleTranscript","Global","ctrl+o"),J;if(q[0]===Symbol.for("react.memo_cache_sentinel"))J=!1,q[0]=J;else J=q[0];if(!H&&!J)return null;if(!1)return null;let j=z,D;if(q[1]!==H)D="\u2234 Thinking",q[1]=H,q[2]=D;else D=q[2];let M=D;if(!1){let Z=$?1:0,N;if(q[3]!==_||q[4]!==M)N=O91.default.createElement(f,{dimColor:!0,italic:!0},M," (",_," to expand)"),q[3]=_,q[4]=M,q[5]=N;else N=q[5];let T;if(q[6]!==Z||q[7]!==N)T=O91.default.createElement(I,{marginTop:Z},N),q[6]=Z,q[7]=N,q[8]=T;else T=q[8];return T}let P=$?1:0,W;if(q[9]!==M)W=O91.default.createElement(f,{color:"warning",bold:!0},"\u{1F351} Thinking Process"),q[9]=M,q[10]=W;else W=q[10];let G;if(q[11]!==H)G=O91.default.createElement(I,{paddingLeft:1,marginTop:1},O91.default.createElement(SJ,null,H)),q[11]=H,q[12]=G;else G=q[12];let V;if(q[13]!==P||q[14]!==W||q[15]!==G)V=O91.default.createElement(I,{flexDirection:"column",borderStyle:"single",borderColor:"warning",paddingX:1,marginTop:P,overflow:"hidden"},W,G),q[13]=P,q[14]=W,q[15]=G,q[16]=V;else V=q[16];return V}';
...
const dW6OldCustomReplacement = 'borderColor:"#FFA500",paddingX:1,marginTop:P,width:"100%"';

function checkPatchStatus(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');

    // Check fyY patch status (the outer gate that controls if thinking blocks render at all)
    const fyyPatched = content.includes(fyYReplacement) && content.includes(fyYThinkingReplacement);
    const fyyNeedsPatching = content.includes(fyYSearchPattern) || content.includes(fyYThinkingSearchPattern);
    ...
    if (content.includes(dW6CustomReplacement)) {
      ...
      } else if (fyyNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'custom-peach-v2-no-fyy', note: 'Has dW6 patch but missing fyY gate patch (thinking hidden by default)' };
      }
    ...
    if (content.includes(dW6OldCustomReplacement) && content.includes('\u{1F351} Thinking Process')) {
    ...
    // Check for any other patch variant (has the if(!1) pattern in dW6 but different styling)
    if (content.includes('function dW6(A)') && content.match(/function dW6\(A\)[^}]*if\(!1\)return null/)) {
      if (fyyNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'partial-no-fyy', note: 'dW6 patched but fyY gate not patched (thinking hidden by default)' };
      }
...
// Old dW6 custom replacement (with hex colors - for upgrading)
const dW6OldCustomReplacementFull = 'function dW6(A){let q=e(17),{param:K,addMargin:Y,isTranscriptMode:z,hideInTranscript:w}=A,{thinking:H}=K,$=Y===void 0?!1:Y,O=w===void 0?!1:w,_=fK("app:toggleTranscript","Global","ctrl+o"),J;if(q[0]===Symbol.for("react.memo_cache_sentinel"))J=!1,q[0]=J;else J=q[0];if(!H&&!J)return null;if(!1)return null;let j=z,D;if(q[1]!==H)D="\u2234 Thinking",q[1]=H,q[2]=D;else D=q[2];let M=D;if(!1){let Z=$?1:0,N;if(q[3]!==_||q[4]!==M)N=O91.default.createElement(f,{dimColor:!0,italic:!0},M," (",_," to expand)"),q[3]=_,q[4]=M,q[5]=N;else N=q[5];let T;if(q[6]!==Z||q[7]!==N)T=O91.default.createElement(I,{marginTop:Z},N),q[6]=Z,q[7]=N,q[8]=T;else T=q[8];return T}let P=$?1:0,W;if(q[9]!==M)W=O91.default.createElement(f,{color:"#FFA500",bold:!0},"\u{1F351} Thinking Process"),q[9]=M,q[10]=W;else W=q[10];let G;if(q[11]!==H)G=O91.default.createElement(I,{paddingLeft:1,marginTop:1},O91.default.createElement(SJ,null,H)),q[11]=H,q[12]=G;else G=q[12];let V;if(q[13]!==P||q[14]!==W||q[15]!==G)V=O91.default.createElement(I,{flexDirection:"column",borderStyle:"single",borderColor:"#FFA500",paddingX:1,marginTop:P,width:"100%"},W,G),q[13]=P,q[14]=W,q[15]=G,q[16]=V;else V=q[16];return V}';
...
function applyPatch(installation) {
  ...
  let modified = false;
  let fyyPatched = false;

  // First, always check and apply fyY patches (the outer gate)
  if (content.includes(fyYSearchPattern)) {
    content = content.replace(fyYSearchPattern, fyYReplacement);
    fyyPatched = true;
    console.log('   \u2705 fyY redacted_thinking gate patched: if(!D&&!Z) -> if(!1)');
  }
  if (content.includes(fyYThinkingSearchPattern)) {
    content = content.replace(fyYThinkingSearchPattern, fyYThinkingReplacement);
    fyyPatched = true;
    console.log('   \u2705 fyY thinking gate patched: if(!D&&!Z) -> if(!1)');
  }

  // Check if dW6 is already fully patched with custom peach style
  if (content.includes(dW6CustomReplacement)) {
    if (fyyPatched) {
      fs.writeFileSync(installation.path, content, 'utf8');
      console.log('   \u2705 fyY gate patches applied (dW6 was already patched)');
      return true;
    }
  ...
  if (content.includes(dW6OldCustomReplacementFull)) {
    content = content.replace(dW6OldCustomReplacementFull, dW6CustomReplacement);
    modified = true;
    console.log('   \u2705 dW6 upgraded from hex colors to theme colors!');
  }
  // Can patch from standard version too
  else if (content.includes(dW6StandardReplacement)) {
    content = content.replace(dW6StandardReplacement, dW6CustomReplacement);
    modified = true;
    console.log('   \u2705 dW6 upgraded from standard to custom peach style!');
  }
  // Patch fresh from original
  else if (content.includes(dW6SearchPattern)) {
    content = content.replace(dW6SearchPattern, dW6CustomReplacement);
    modified = true;
    console.log('   \u2705 dW6 patch applied!');
  ...
  if (modified || fyyPatched) {
    fs.writeFileSync(installation.path, content, 'utf8');
    if (!modified && fyyPatched) {
      console.log('   \u2705 fyY gate patches applied successfully!');
    }
    return true;
  } else {
...
async function main() {
  console.log('Claude Code Thinking Custom Peach Style Patcher v2.1.44');
  console.log('========================================================');
