...
// ============================================================
// Claude Code Thinking Display Patcher v2.1.121 (Custom Peach)
//
...
//
// Identifiers: K96 (component), c$H (React), p (Box), v (Text),
//   fY (ThinkingContent), VP7 (cache, 9 slots), Uf (Spinner)
// Gate function: dW5 — thinking + redacted_thinking cases patched
// Redact-thinking beta header variable: CI_
// ============================================================
...
// Same-length byte patches for v2.1.121. All `search`/`replace` pairs must
// have identical byte lengths — the binary's Mach-O layout depends on offsets.
const PATCHES = [
  {
    name: 'Redact-thinking beta header disabled',
    search: Buffer.from('if(O&&e89(H)&&!S8()&&I8().showThinkingSummaries!==!0)_.push(CI_);'),
    replace: Buffer.from('if(0&&e89(H)&&!S8()&&I8().showThinkingSummaries!==!0)_.push(CI_);')
  },
  {
    name: 'K96 component always renders expanded',
    search: Buffer.from('function K96(H){let _=VP7.c(9),{param:q,addMargin:K,isTranscriptMode:O,verbose:T,hideInTranscript:A}=H,{thinking:$}=q,z=K===void 0?!1:K,Y=A===void 0?!1:A;if(!$)return null;if(Y)return null;if(!(O||T)){'),
    replace: Buffer.from('function K96(H){let _=VP7.c(9),{param:q,addMargin:K,isTranscriptMode:O,verbose:T,hideInTranscript:A}=H,{thinking:$}=q,z=K===void 0?!1:K,Y=A===void 0?!1:A;if(!$)return null;if(0)return null;if(!(1||1)){')
  },
  {
    name: 'dW5 gate — redacted_thinking case always renders',
    search: Buffer.from('case"redacted_thinking":{if(!J&&!A)return null;'),
    replace: Buffer.from('case"redacted_thinking":{if(!1&&!1)return null;')
  },
  {
    name: 'dW5 gate — thinking case always renders',
    search: Buffer.from('case"thinking":{if(!J&&!A)return null;'),
    replace: Buffer.from('case"thinking":{if(!1&&!1)return null;')
  },
  {
    // v2.1.121 API behavior: if thinking.display is unset, server defaults to
    ...
    search:  Buffer.from('$_=U_?q.display:void 0'),
  ...
  },
  {
    ...
    // Note: in v2.1.121 the destructuring swapped variable names vs v2.1.119:
    // thinking is `$` (was `A`), thinking element is `J` (was `f`),
    // final return element is `f` (was `J`). All single-char so byte-equal.
    name: 'K96 visual styling — peach border + warning colors',
    search: Buffer.from(
      'let M=z?1:0,P;if(_[0]===Symbol.for("react.memo_cache_sentinel"))' +
      'P=c$H.default.createElement(v,{dimColor:!0,italic:!0},"\\u2234 Thinking"," ",' +
      'c$H.default.createElement(Uf,null)),_[0]=P;else P=_[0];' +
      'let X;if(_[1]!==M)X=c$H.default.createElement(p,{marginTop:M},P),_[1]=M,_[2]=X;else X=_[2];return X}' +
      'let D=z?1:0,j;if(_[3]===Symbol.for("react.memo_cache_sentinel"))' +
      'j=c$H.default.createElement(v,{dimColor:!0,italic:!0},"\\u2234 Thinking","\\u2026"),_[3]=j;else j=_[3];' +
      'let J;if(_[4]!==$)J=c$H.default.createElement(p,{paddingLeft:2},' +
      'c$H.default.createElement(fY,{dimColor:!0},$)),_[4]=$,_[5]=J;else J=_[5];' +
      'let f;if(_[6]!==D||_[7]!==J)f=c$H.default.createElement(p,' +
      '{flexDirection:"column",gap:1,marginTop:D,width:"100%"},j,J),' +
      '_[6]=D,_[7]=J,_[8]=f;else f=_[8];return f}'
    ),
    replace: Buffer.from(
      'let M=z?1:0,P;if(_[0]===Symbol.for("react.memo_cache_sentinel"))' +
      'P=c$H.default.createElement(v,{c:!0},"\\u2234 Thinking"," ",' +
      'c$H.default.createElement(Uf,null)),_[0]=P;else P=_[0];' +
      'let X;if(_[1]!==M)X=c$H.default.createElement(p,{c:M},P),_[1]=M,_[2]=X;else X=_[2];return X}' +
      'let D=z?1:0,j;if(_[3]===Symbol.for("react.memo_cache_sentinel"))' +
      'j=c$H.default.createElement(v,{color:"warning",bold:!0},"\\ud83c\\udf51 Thinking"),_[3]=j;else j=_[3];' +
      'let J;if(_[4]!==$)J=c$H.default.createElement(p,{paddingLeft:2},' +
      'c$H.default.createElement(fY,null,$)),_[4]=$,_[5]=J;else J=_[5];' +
      'let f;if(_[6]!==D||_[7]!==J)f=c$H.default.createElement(p,' +
      '{flexDirection:"column",borderStyle:"single",borderColor:"warning",paddingX:1,marginTop:D},j,J),' +
      '_[6]=D,_[7]=J,_[8]=f;else f=_[8];return f}'
    )
  }
];
...
function checkPatchStatus(inst) {
  try {
    ...
    const marker = Buffer.from('function K96(H){let _=VP7.c(9)');
    if (data.indexOf(marker) === -1) {
      return { status: 'unknown', canPatch: false, reason: 'v2.1.121 K96 signature not found — wrong version?' };
    }
...
async function main() {
  console.log('Claude Code Thinking Display Patcher v2.1.121 (Custom Peach)');
  console.log('============================================================');
  ...
  if (installations.length === 0) {
    ...
    console.error('Install v2.1.121 via: npm i -g @anthropic-ai/claude-code');
    process.exit(1);
  }
  ...
  console.log('   - K96 component always renders expanded thinking');
  console.log('   - dW5 gate patched: thinking & redacted_thinking render in default mode');
