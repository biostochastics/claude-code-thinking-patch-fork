...
// ============================================================
// Claude Code Thinking Display Patcher v2.1.112 (Standard)
...
function checkPatchStatus(filePath) {
  try {
    ...
    if (content.includes('if(!1)return null;') && content.includes('function cg8(q)') && content.includes('borderStyle')) {
    ...
    if (content.includes('function oF8(q)')) {
      return { status: 'unknown', canPatch: false, reason: 'Version v2.1.109 detected - use patch-thinking-v2.1.109.js' };
    }
    ...
    if (content.includes('function Ng8(q)')) {
      return { status: 'unknown', canPatch: false, reason: 'Version v2.1.104 detected - use patch-thinking-v2.1.104.js' };
    }
...
function applyPatch(installation) {
  ...
  if (content.includes(cg8SearchPattern)) {
    content = content.replace(cg8SearchPattern, cg8StandardReplacement);
    modified = true;
...
async function main() {
  console.log('Claude Code Thinking Display Patcher v2.1.112 (Standard)');
  console.log('========================================================');
