...
// ============================================================
// Claude Code Thinking Display Patcher v2.1.121 (Standard)
//
...
// Gate function: dW5 (not patched in standard — see -custom-peach)
...
// Same-length byte patches for v2.1.121 binary.
...
function checkPatchStatus(inst) {
  try {
    ...
    const v2121Marker = Buffer.from('function K96(H){let _=VP7.c(9)');
    if (data.indexOf(v2121Marker) === -1) {
...
async function main() {
  console.log('Claude Code Thinking Display Patcher v2.1.121 (Standard)');
  console.log('========================================================');
  ...
  console.log('   - K96 component always renders expanded thinking (no compact spinner view)');
  console.log('\nNote: The outer dW5 gate still returns null in default mode — thinking');
  console.log('      blocks appear only in transcript mode (ctrl+o) or with --verbose.');
  console.log('      For default-mode visibility use patch-thinking-v2.1.121-custom-peach.js.');
}
