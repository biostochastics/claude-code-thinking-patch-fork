...
// ============================================================
// Claude Code Thinking Standard Patcher v2.1.19
...
function checkPatchStatus(filePath) {
  try {
    ...
    if (content.includes(oG1Replacement)) {
      return { status: 'patched', canPatch: false, variant: 'standard' };
    }

    // Check for custom styled patch (orange border, peach emoji)
    if (content.includes('if(!1)return null;') && content.includes('function oG1(A)') && content.includes('🍑 Thinking Process')) {
...
function applyPatch(installation) {
  ...
  if (content.includes(oG1SearchPattern)) {
    content = content.replace(oG1SearchPattern, oG1Replacement);
...
async function main() {
  console.log('Claude Code Thinking Standard Patcher v2.1.19');
  console.log('==============================================');
