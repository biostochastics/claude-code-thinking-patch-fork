...
// ============================================================
// Claude Code Thinking Custom Peach Style Patcher v2.1.104
...
// eOY function patterns - this function gates whether thinking blocks are rendered at all
...
const eOYSearchPattern = 'case"redacted_thinking":{if(!M&&!O)return null;';
const eOYReplacement = 'case"redacted_thinking":{if(!1)return null;';
const eOYThinkingSearchPattern = 'case"thinking":{if(!M&&!O)return null;';
const eOYThinkingReplacement = 'case"thinking":{if(!1)return null;';
...
const Ng8CustomReplacement = 'function Ng8(q){let K=_6(9),{param:_,addMargin:z,isTranscriptMode:Y,verbose:A,hideInTranscript:O}=q,{thinking:w}=_,$=z===void 0?!1:z,j=O===void 0?!1:O;if(!w)return null;if(!1)return null;if(!1){let W=$?1:0,D;if(K[0]===Symbol.for("react.memo_cache_sentinel"))D=O96.default.createElement(V,{dimColor:!0,italic:!0},"\u2234 Thinking"," ",O96.default.createElement(Uw,null)),K[0]=D;else D=K[0];let Z;if(K[1]!==W)Z=O96.default.createElement(u,{marginTop:W},D),K[1]=W,K[2]=Z;else Z=K[2];return Z}let J=$?1:0,X;if(K[3]===Symbol.for("react.memo_cache_sentinel"))X=O96.default.createElement(V,{color:"warning",bold:!0},"\ud83c\udf51 Thinking Process"),K[3]=X;else X=K[3];let M;if(K[4]!==w)M=O96.default.createElement(u,{paddingLeft:1,marginTop:1},O96.default.createElement(S2,null,w)),K[4]=w,K[5]=M;else M=K[5];let P;if(K[6]!==J||K[7]!==M)P=O96.default.createElement(u,{flexDirection:"column",borderStyle:"single",borderColor:"warning",paddingX:1,marginTop:J,overflow:"hidden"},X,M),K[6]=J,K[7]=M,K[8]=P;else P=K[8];return P}';
...
const Ng8OldCustomReplacement = 'borderColor:"#FFA500",paddingX:1,marginTop:J,width:"100%"';
...
function checkPatchStatus(filePath) {
  try {
    ...
    // Check eOY patch status (the outer gate that controls if thinking blocks render at all)
    const eOYPatched = content.includes(eOYReplacement) && content.includes(eOYThinkingReplacement);
    const eOYNeedsPatching = content.includes(eOYSearchPattern) || content.includes(eOYThinkingSearchPattern);

    // Check for new custom peach patched version (this script's target with theme colors)
    if (content.includes(Ng8CustomReplacement)) {
      if (eOYPatched && redactPatched) {
        return { status: 'patched', canPatch: false, variant: 'custom-peach-v3' };
      } else if (eOYNeedsPatching || redactNeedsPatching) {
        const missing = [];
        if (eOYNeedsPatching) missing.push('eOY gate');
        if (redactNeedsPatching) missing.push('redact-thinking header');
        return { status: 'patched-other', canPatch: true, variant: 'custom-peach-incomplete', note: `Has Ng8 patch but missing: ${missing.join(', ')}` };
      }
    ...
    if (content.includes(Ng8OldCustomReplacement) && content.includes('\ud83c\udf51 Thinking Process')) {
    ...
    // Check for any other patch variant (has the if(!1) pattern in Ng8 but different styling)
    if (content.includes('function Ng8(q)') && content.match(/function Ng8\(q\)[^}]*if\(!1\)return null/)) {
      if (eOYNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'partial-no-eOY', note: 'Ng8 patched but eOY gate not patched (thinking hidden by default)' };
      }
...
// Old Ng8 custom replacement (with hex colors - for upgrading)
const Ng8OldCustomReplacementFull = 'function Ng8(q){let K=_6(9),{param:_,addMargin:z,isTranscriptMode:Y,verbose:A,hideInTranscript:O}=q,{thinking:w}=_,$=z===void 0?!1:z,j=O===void 0?!1:O;if(!w)return null;if(!1)return null;if(!1){let W=$?1:0,D;if(K[0]===Symbol.for("react.memo_cache_sentinel"))D=O96.default.createElement(V,{dimColor:!0,italic:!0},"\u2234 Thinking"," ",O96.default.createElement(Uw,null)),K[0]=D;else D=K[0];let Z;if(K[1]!==W)Z=O96.default.createElement(u,{marginTop:W},D),K[1]=W,K[2]=Z;else Z=K[2];return Z}let J=$?1:0,X;if(K[3]===Symbol.for("react.memo_cache_sentinel"))X=O96.default.createElement(V,{color:"#FFA500",bold:!0},"\ud83c\udf51 Thinking Process"),K[3]=X;else X=K[3];let M;if(K[4]!==w)M=O96.default.createElement(u,{paddingLeft:1,marginTop:1},O96.default.createElement(S2,null,w)),K[4]=w,K[5]=M;else M=K[5];let P;if(K[6]!==J||K[7]!==M)P=O96.default.createElement(u,{flexDirection:"column",borderStyle:"single",borderColor:"#FFA500",paddingX:1,marginTop:J,width:"100%"},X,M),K[6]=J,K[7]=M,K[8]=P;else P=K[8];return P}';
...
function applyPatch(installation) {
  ...
  // Next, always check and apply eOY patches (the outer gate)
  if (content.includes(eOYSearchPattern)) {
    content = content.replace(eOYSearchPattern, eOYReplacement);
    eOYPatchApplied = true;
    console.log('   \u2705 eOY redacted_thinking gate patched: if(!M&&!O) -> if(!1)');
  }
  if (content.includes(eOYThinkingSearchPattern)) {
    content = content.replace(eOYThinkingSearchPattern, eOYThinkingReplacement);
    eOYPatchApplied = true;
    console.log('   \u2705 eOY thinking gate patched: if(!M&&!O) -> if(!1)');
  }

  // Check if Ng8 is already fully patched with custom peach style
  if (content.includes(Ng8CustomReplacement)) {
    if (eOYPatchApplied || modified) {
      fs.writeFileSync(installation.path, content, 'utf8');
      console.log('   \u2705 Gate/redact patches applied (Ng8 was already patched)');
      return true;
    }
  ...
  if (content.includes(Ng8OldCustomReplacementFull)) {
    content = content.replace(Ng8OldCustomReplacementFull, Ng8CustomReplacement);
    modified = true;
    console.log('   \u2705 Ng8 upgraded from hex colors to theme colors!');
  }
  // Can patch from standard version too
  else if (content.includes(Ng8StandardReplacement)) {
    content = content.replace(Ng8StandardReplacement, Ng8CustomReplacement);
    modified = true;
    console.log('   \u2705 Ng8 upgraded from standard to custom peach style!');
  }
  // Patch fresh from original
  else if (content.includes(Ng8SearchPattern)) {
    content = content.replace(Ng8SearchPattern, Ng8CustomReplacement);
    modified = true;
  ...
  if (modified || eOYPatchApplied) {
    fs.writeFileSync(installation.path, content, 'utf8');
    if (!modified && eOYPatchApplied) {
      console.log('   \u2705 eOY gate patches applied successfully!');
    }
    return true;
  } else {
...
async function main() {
  console.log('Claude Code Thinking Custom Peach Style Patcher v2.1.104');
  console.log('========================================================');
