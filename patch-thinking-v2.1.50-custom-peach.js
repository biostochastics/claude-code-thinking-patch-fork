...
// ============================================================
// Claude Code Thinking Custom Peach Style Patcher v2.1.50
...
// Gate function: jiY (gates whether thinking blocks render at all)
...
// jiY function patterns - this function gates whether thinking blocks are rendered at all
// The check `if(!X&&!T&&!_)return null` blocks rendering when not in transcript mode
// We need to change both occurrences (for redacted_thinking and thinking cases) to if(!1)
const jiYSearchPattern = 'case"redacted_thinking":{if(!X&&!T&&!_)return null;';
const jiYReplacement = 'case"redacted_thinking":{if(!1)return null;';
const jiYThinkingSearchPattern = 'case"thinking":{if(!X&&!T&&!_)return null;';
const jiYThinkingReplacement = 'case"thinking":{if(!1)return null;';

// Custom replacement with peach emoji, orange border, and bold header
// Changes: if(O) -> if(!1), if(!X) -> if(!1), plus custom styling
// Uses "warning" theme color for borders (orange-ish) and overflow:hidden for proper rendering
const rT1CustomReplacement = 'function rT1(A){let q=K6(16),{param:K,addMargin:Y,isTranscriptMode:z,verbose:w,hideInTranscript:_}=A,{thinking:$}=K,H=Y===void 0?!1:Y,O=_===void 0?!1:_,J=GK("app:toggleTranscript","Global","ctrl+o"),j;if(q[0]===Symbol.for("react.memo_cache_sentinel"))j=!1,q[0]=j;else j=q[0];if(!$&&!j)return null;if(!1)return null;let X=z||w,M;if(q[1]!==$)M="\u2234 Thinking",q[1]=$,q[2]=M;else M=q[2];let P=M;if(!1){let V=H?1:0,N=`${P} (${J} to expand)`,v;if(q[3]!==N)v=LY6.default.createElement(f,{dimColor:!0,italic:!0},N),q[3]=N,q[4]=v;else v=q[4];let y;if(q[5]!==V||q[6]!==v)y=LY6.default.createElement(b,{marginTop:V},v),q[5]=V,q[6]=v,q[7]=y;else y=q[7];return y}let W=H?1:0,Z;if(q[8]!==P)Z=LY6.default.createElement(f,{color:"warning",bold:!0},"\ud83c\udf51 Thinking Process"),q[8]=P,q[9]=Z;else Z=q[9];let G;if(q[10]!==$)G=LY6.default.createElement(b,{paddingLeft:1,marginTop:1},LY6.default.createElement(WO,null,$)),q[10]=$,q[11]=G;else G=q[11];let T;if(q[12]!==W||q[13]!==Z||q[14]!==G)T=LY6.default.createElement(b,{flexDirection:"column",borderStyle:"single",borderColor:"warning",paddingX:1,marginTop:W,overflow:"hidden"},Z,G),q[12]=W,q[13]=Z,q[14]=G,q[15]=T;else T=q[15];return T}';
...
const rT1OldCustomReplacement = 'borderColor:"#FFA500",paddingX:1,marginTop:W,width:"100%"';

function checkPatchStatus(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');

    // Check jiY patch status (the outer gate that controls if thinking blocks render at all)
    const jiyPatched = content.includes(jiYReplacement) && content.includes(jiYThinkingReplacement);
    const jiyNeedsPatching = content.includes(jiYSearchPattern) || content.includes(jiYThinkingSearchPattern);

    // Check for new custom peach patched version (this script's target with theme colors)
    if (content.includes(rT1CustomReplacement)) {
      if (jiyPatched) {
        return { status: 'patched', canPatch: false, variant: 'custom-peach-v3' };
      } else if (jiyNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'custom-peach-v2-no-jiy', note: 'Has rT1 patch but missing jiY gate patch (thinking hidden by default)' };
      }
    ...
    if (content.includes(rT1OldCustomReplacement) && content.includes('\ud83c\udf51 Thinking Process')) {
    ...
    // Check for any other patch variant (has the if(!1) pattern in rT1 but different styling)
    if (content.includes('function rT1(A)') && content.match(/function rT1\(A\)[^}]*if\(!1\)return null/)) {
      if (jiyNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'partial-no-jiy', note: 'rT1 patched but jiY gate not patched (thinking hidden by default)' };
      }
...
// Old rT1 custom replacement (with hex colors - for upgrading)
const rT1OldCustomReplacementFull = 'function rT1(A){let q=K6(16),{param:K,addMargin:Y,isTranscriptMode:z,verbose:w,hideInTranscript:_}=A,{thinking:$}=K,H=Y===void 0?!1:Y,O=_===void 0?!1:_,J=GK("app:toggleTranscript","Global","ctrl+o"),j;if(q[0]===Symbol.for("react.memo_cache_sentinel"))j=!1,q[0]=j;else j=q[0];if(!$&&!j)return null;if(!1)return null;let X=z||w,M;if(q[1]!==$)M="\u2234 Thinking",q[1]=$,q[2]=M;else M=q[2];let P=M;if(!1){let V=H?1:0,N=`${P} (${J} to expand)`,v;if(q[3]!==N)v=LY6.default.createElement(f,{dimColor:!0,italic:!0},N),q[3]=N,q[4]=v;else v=q[4];let y;if(q[5]!==V||q[6]!==v)y=LY6.default.createElement(b,{marginTop:V},v),q[5]=V,q[6]=v,q[7]=y;else y=q[7];return y}let W=H?1:0,Z;if(q[8]!==P)Z=LY6.default.createElement(f,{color:"#FFA500",bold:!0},"\ud83c\udf51 Thinking Process"),q[8]=P,q[9]=Z;else Z=q[9];let G;if(q[10]!==$)G=LY6.default.createElement(b,{paddingLeft:1,marginTop:1},LY6.default.createElement(WO,null,$)),q[10]=$,q[11]=G;else G=q[11];let T;if(q[12]!==W||q[13]!==Z||q[14]!==G)T=LY6.default.createElement(b,{flexDirection:"column",borderStyle:"single",borderColor:"#FFA500",paddingX:1,marginTop:W,width:"100%"},Z,G),q[12]=W,q[13]=Z,q[14]=G,q[15]=T;else T=q[15];return T}';
...
function applyPatch(installation) {
  ...
  let modified = false;
  let jiyPatched = false;

  // First, always check and apply jiY patches (the outer gate)
  if (content.includes(jiYSearchPattern)) {
    content = content.replace(jiYSearchPattern, jiYReplacement);
    jiyPatched = true;
    console.log('   \u2705 jiY redacted_thinking gate patched: if(!X&&!T&&!_) -> if(!1)');
  }
  if (content.includes(jiYThinkingSearchPattern)) {
    content = content.replace(jiYThinkingSearchPattern, jiYThinkingReplacement);
    jiyPatched = true;
    console.log('   \u2705 jiY thinking gate patched: if(!X&&!T&&!_) -> if(!1)');
  }

  // Check if rT1 is already fully patched with custom peach style
  if (content.includes(rT1CustomReplacement)) {
    if (jiyPatched) {
      fs.writeFileSync(installation.path, content, 'utf8');
      console.log('   \u2705 jiY gate patches applied (rT1 was already patched)');
      return true;
    }
  ...
  if (content.includes(rT1OldCustomReplacementFull)) {
    content = content.replace(rT1OldCustomReplacementFull, rT1CustomReplacement);
    modified = true;
    console.log('   \u2705 rT1 upgraded from hex colors to theme colors!');
  }
  // Can patch from standard version too
  else if (content.includes(rT1StandardReplacement)) {
    content = content.replace(rT1StandardReplacement, rT1CustomReplacement);
    modified = true;
    console.log('   \u2705 rT1 upgraded from standard to custom peach style!');
  }
  // Patch fresh from original
  else if (content.includes(rT1SearchPattern)) {
    content = content.replace(rT1SearchPattern, rT1CustomReplacement);
    modified = true;
    console.log('   \u2705 rT1 patch applied!');
  ...
  if (modified || jiyPatched) {
    fs.writeFileSync(installation.path, content, 'utf8');
    if (!modified && jiyPatched) {
      console.log('   \u2705 jiY gate patches applied successfully!');
    }
    return true;
  } else {
...
async function main() {
  console.log('Claude Code Thinking Custom Peach Style Patcher v2.1.50');
  console.log('========================================================');
