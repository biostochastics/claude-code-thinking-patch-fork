...
/**
 * Claude Code Thinking Custom Style Patcher v2.0.29
 *
 * This patch adds custom peach-themed styling to thinking blocks in Claude Code v2.0.29.
 * It applies three patches:
 * 1. Makes thinking blocks visible
 * 2. Adds custom border and colors (peach/orange theme)
 *
 * Identified Patterns for v2.0.29:
 ...
 *
 * Usage:
 *   node patch-thinking-v2.0.29-custom.js
 */
...
console.log('Claude Code Thinking Custom Style Patcher v2.0.29');
console.log('==================================================\n');
...
if (!fs.existsSync(claudePath)) {
  ...
  console.error('Please make sure Claude Code v2.0.29 is installed.');
  process.exit(1);
}
...
if (versionMatch) {
  ...
  if (version !== '2.0.29') {
    console.error(`\n⚠️  Warning: This patch is for v2.0.29, but found v${version}`);
...
// Patch 1: Thinking Visibility (v2.0.29)
...
// Patch 2: Custom LTQ styling with border and peach color (v2.0.29)
...
if (content.includes(ltqSearchPattern)) {
  ...
  console.log('   1. Wrong Claude Code version (this is for v2.0.29)');
...
if (patch1Applied || patch2Applied) {
  ...
  console.error('This script is for Claude Code v2.0.29');
  process.exit(1);
}
