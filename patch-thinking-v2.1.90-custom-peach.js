...
// ============================================================
// Claude Code Thinking Custom Peach Style Patcher v2.1.90
...
// Udz function patterns - this function gates whether thinking blocks are rendered at all
...
const UdzSearchPattern = 'case"redacted_thinking":{if(!X&&!O)return null;';
const UdzReplacement = 'case"redacted_thinking":{if(!1)return null;';
const UdzThinkingSearchPattern = 'case"thinking":{if(!X&&!O)return null;';
const UdzThinkingReplacement = 'case"thinking":{if(!1)return null;';
...
const AI8CustomReplacement = 'function AI8(q){let K=Y6(9),{param:_,addMargin:z,isTranscriptMode:Y,verbose:$,hideInTranscript:O}=q,{thinking:A}=_,w=z===void 0?!1:z,j=O===void 0?!1:O;if(!A)return null;if(!1)return null;if(!1){let W=w?1:0,D;if(K[0]===Symbol.for("react.memo_cache_sentinel"))D=bK6.default.createElement(T,{dimColor:!0,italic:!0},"\u2234 Thinking"," ",bK6.default.createElement(X2,null)),K[0]=D;else D=K[0];let f;if(K[1]!==W)f=bK6.default.createElement(u,{marginTop:W},D),K[1]=W,K[2]=f;else f=K[2];return f}let J=w?1:0,M;if(K[3]===Symbol.for("react.memo_cache_sentinel"))M=bK6.default.createElement(T,{color:"warning",bold:!0},"\ud83c\udf51 Thinking Process"),K[3]=M;else M=K[3];let X;if(K[4]!==A)X=bK6.default.createElement(u,{paddingLeft:1,marginTop:1},bK6.default.createElement(DA,null,A)),K[4]=A,K[5]=X;else X=K[5];let P;if(K[6]!==J||K[7]!==X)P=bK6.default.createElement(u,{flexDirection:"column",borderStyle:"single",borderColor:"warning",paddingX:1,marginTop:J,overflow:"hidden"},M,X),K[6]=J,K[7]=X,K[8]=P;else P=K[8];return P}';
...
const AI8OldCustomReplacement = 'borderColor:"#FFA500",paddingX:1,marginTop:J,width:"100%"';
...
function checkPatchStatus(filePath) {
  try {
    ...
    // Check Udz patch status (the outer gate that controls if thinking blocks render at all)
    const UdzPatched = content.includes(UdzReplacement) && content.includes(UdzThinkingReplacement);
    const UdzNeedsPatching = content.includes(UdzSearchPattern) || content.includes(UdzThinkingSearchPattern);
    ...
    if (content.includes(AI8CustomReplacement)) {
      if (UdzPatched && redactPatched) {
        return { status: 'patched', canPatch: false, variant: 'custom-peach-v3' };
      } else if (UdzNeedsPatching || redactNeedsPatching) {
        const missing = [];
        if (UdzNeedsPatching) missing.push('Udz gate');
        if (redactNeedsPatching) missing.push('redact-thinking header');
        return { status: 'patched-other', canPatch: true, variant: 'custom-peach-incomplete', note: `Has AI8 patch but missing: ${missing.join(', ')}` };
      }
    ...
    if (content.includes(AI8OldCustomReplacement) && content.includes('\ud83c\udf51 Thinking Process')) {
    ...
    // Check for any other patch variant (has the if(!1) pattern in AI8 but different styling)
    if (content.includes('function AI8(q)') && content.match(/function AI8\(q\)[^}]*if\(!1\)return null/)) {
      if (UdzNeedsPatching) {
        return { status: 'patched-other', canPatch: true, variant: 'partial-no-Udz', note: 'AI8 patched but Udz gate not patched (thinking hidden by default)' };
      }
...
// Old AI8 custom replacement (with hex colors - for upgrading)
const AI8OldCustomReplacementFull = 'function AI8(q){let K=Y6(9),{param:_,addMargin:z,isTranscriptMode:Y,verbose:$,hideInTranscript:O}=q,{thinking:A}=_,w=z===void 0?!1:z,j=O===void 0?!1:O;if(!A)return null;if(!1)return null;if(!1){let W=w?1:0,D;if(K[0]===Symbol.for("react.memo_cache_sentinel"))D=bK6.default.createElement(T,{dimColor:!0,italic:!0},"\u2234 Thinking"," ",bK6.default.createElement(X2,null)),K[0]=D;else D=K[0];let f;if(K[1]!==W)f=bK6.default.createElement(u,{marginTop:W},D),K[1]=W,K[2]=f;else f=K[2];return f}let J=w?1:0,M;if(K[3]===Symbol.for("react.memo_cache_sentinel"))M=bK6.default.createElement(T,{color:"#FFA500",bold:!0},"\ud83c\udf51 Thinking Process"),K[3]=M;else M=K[3];let X;if(K[4]!==A)X=bK6.default.createElement(u,{paddingLeft:1,marginTop:1},bK6.default.createElement(DA,null,A)),K[4]=A,K[5]=X;else X=K[5];let P;if(K[6]!==J||K[7]!==X)P=bK6.default.createElement(u,{flexDirection:"column",borderStyle:"single",borderColor:"#FFA500",paddingX:1,marginTop:J,width:"100%"},M,X),K[6]=J,K[7]=X,K[8]=P;else P=K[8];return P}';
...
function applyPatch(installation) {
  ...
  // Next, always check and apply Udz patches (the outer gate)
  if (content.includes(UdzSearchPattern)) {
    content = content.replace(UdzSearchPattern, UdzReplacement);
    UdzPatchApplied = true;
    console.log('   \u2705 Udz redacted_thinking gate patched: if(!X&&!O) -> if(!1)');
  }
  if (content.includes(UdzThinkingSearchPattern)) {
    content = content.replace(UdzThinkingSearchPattern, UdzThinkingReplacement);
    UdzPatchApplied = true;
    console.log('   \u2705 Udz thinking gate patched: if(!X&&!O) -> if(!1)');
  }

  // Check if AI8 is already fully patched with custom peach style
  if (content.includes(AI8CustomReplacement)) {
    if (UdzPatchApplied || modified) {
      fs.writeFileSync(installation.path, content, 'utf8');
      console.log('   \u2705 Gate/redact patches applied (AI8 was already patched)');
      return true;
    }
  ...
  if (content.includes(AI8OldCustomReplacementFull)) {
    content = content.replace(AI8OldCustomReplacementFull, AI8CustomReplacement);
    modified = true;
    console.log('   \u2705 AI8 upgraded from hex colors to theme colors!');
  }
  // Can patch from standard version too
  else if (content.includes(AI8StandardReplacement)) {
    content = content.replace(AI8StandardReplacement, AI8CustomReplacement);
    modified = true;
    console.log('   \u2705 AI8 upgraded from standard to custom peach style!');
  }
  ...
  if (modified || UdzPatchApplied) {
    fs.writeFileSync(installation.path, content, 'utf8');
    if (!modified && UdzPatchApplied) {
      console.log('   \u2705 Udz gate patches applied successfully!');
    }
    return true;
  } else {
...
async function main() {
  console.log('Claude Code Thinking Custom Peach Style Patcher v2.1.90');
  console.log('========================================================');
