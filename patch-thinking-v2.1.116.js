...
// ============================================================
// Claude Code Thinking Display Patcher v2.1.116 (Standard)
//
...
// Gate function: RH5 (not patched in standard — see -custom-peach)
...
// Same-length byte patches for v2.1.116 binary.
...
function checkPatchStatus(inst) {
  try {
    ...
    const v2116Marker = Buffer.from('function Vi_(H){let _=u97.c(9)');
    if (data.indexOf(v2116Marker) === -1) {
...
async function main() {
  console.log('Claude Code Thinking Display Patcher v2.1.116 (Standard)');
  console.log('========================================================');
  ...
  console.log('   - Vi_ component always renders expanded thinking (no compact spinner view)');
  console.log('\nNote: The outer RH5 gate still returns null in default mode — thinking');
  console.log('      blocks appear only in transcript mode (ctrl+o) or with --verbose.');
  console.log('      For default-mode visibility use patch-thinking-v2.1.116-custom-peach.js.');
}
